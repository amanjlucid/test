...
export class WorkorderListComponent implements OnInit {
  ...
  ngOnDestroy() {
    ...
    getUserWorksOrdersList(userId, hideComplete)
     {
      this.subs.add(
        this.eveneManagerService.getListOfUserWorksOrderByUserId(userId, 'A').subscribe(
          data => {
            if (data.isSuccess) {
            ...
            // alert(data.data.length);
              for (let k = 0; k < data.data.length; k++) {
                ...
                if(k == 0){
                  ...
                  //  console.log(JSON.stringify(data.data[k] ));
                }
     /*
                 if(data.data[k].eventRowCount > 10)
                  data.data[k].eventRowCount = '10 se bda';

                  if(data.data[k].eventRowCount > 200)
                   data.data[k].eventRowCount = '200 se bda';
                   */
              }
              // console.log(data)
