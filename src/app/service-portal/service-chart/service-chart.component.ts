...
import { ServicePortalService, AlertService, HelperService, SharedService } from '../../_services'
...
export class ServiceChartComponent implements OnInit {
    ...
    setTimeout(() => {
      this.subs.add(
        this.sharedServie.servicePortalObs.subscribe(data => {
          ...
          this.servicePortalService.getUserChartData(this.currentUser.userId, "Servicing").subscribe(
            data => {
              ...
              this.subs.add(
                this.servicePortalService.getChartType().subscribe(
                  data => {
                    if (data.isSuccess) {
  ...
  renderChartTypes(chartObj: any, classRefer: any, container: any, state: any, cl = null) {
    chartObj.ChartParameterValue = chartObj.chartParameters == 1 ? "0_OPTIVO:CONTRACT1:OPTGAS2" : "";
    if (chartObj.chartType == 3) {
      ...
      chartObj.ChartParameterValue = "0_OPTIVO:CONTRACT1:OPTGAS2";
  ...
  renderLineChartIfStatesaved(container: any, state: any, classRefer: any, cl: any = null) {
    ...
    chartObj.ChartParameterValue = (state.selectedFilter != null && state.selectedFilter != "") ? state.selectedFilter : "0_OPTIVO:CONTRACT1:OPTGAS2";

    this.subs.add(
      this.servicePortalService.getChartData(chartObj).subscribe(
        data => {
          if (data.isSuccess) {
    ...
    //   let optHtml = "";
    //   for (var val in state.filters) {
    //     let v = state.filters[val]
    //     let selectOpt = (state.selectedFilter == v) ? "selected" : "";
    //     optHtml += (`<option ${selectOpt} value="${v}">${v}</option>`);
    //   }
    //   selectElm.html(optHtml);
    ...
    // setTimeout(() => {
    //   let lineChart = this.lineChartInit(null, null, className, state.chartData, state.xaxis);
    //   if (state.filters != null) {
    //     $("." + filterDivCl).html(selectElm);
    //   }
  ...
  getLineChartData(dataForChart: any, className: any, container: any, state: any, chartObj: any) {
    this.subs.add(
      this.servicePortalService.getChartData(dataForChart).subscribe(
        data => {
          if (data.isSuccess) {
            let lineChartData = data.data.lineChartModel;
            //let lineChartFilterData = data.data.chartFilterModel;
  ...
  renderPieChartIfStatesaved(container: any, state: any, classRefer: any, cl: any = null) {
    ...
    // container.getElement().html(`<div class="row" style="width:100%; height:${dynamicCss.parentDiv}%;"><input type="hidden" value="${chartObj.chartName}" class="line${className}"><div class="col-md-12"><div class="${filterDivCl}"></div><div id="${className}" style="width:100%; height: ${dynamicCss.childDiv}%; position:absolute;  margin-top:${dynamicCss.childDivMt}px;"></div> </div></div>`);
    // setTimeout(() => {
    //   classRefer.pieChartInit(null, null, true, className, state.chartData);
    //   if (state.filters !== null) {
    //     $("." + filterDivCl).html(selectElm);
    //   }
  ...
  renderBarChartIfStatesaved(container: any, state: any, classRefer: any, cl: any = null) {
    ...
    // setTimeout(() => {
    //   classRefer.barChartInit(null, null, true, className, state.chartData);
    //   if (state.filters != null) {
    //     $("." + filterDivCl).html(selectElm);
    //   }
  ...
  saveChartsState() {
    ...
    const chartState = { UserId: this.currentUser.userId, ChartData: state, dashboard: "Servicing" }
    this.subs.add(
      this.servicePortalService.saveUserChartData(chartState).subscribe(
        data => {
          if (data.isSuccess) {
            this.dashboardName = "Servicing";
  ...
  // async getChartFilterValue(chartName: string) {
  //   let response: any = await fetch(`${appConfig.apiUrl}/api/Chart/GetChartExampleDataKeys?chartName=${chartName}`);
  //   let data = await response.json();
  //   return Promise.resolve(data.data[0]['filterString']);
  // }

  // getChartDropDownVlaues(chartName) {
  //   let res: any = [];
  //   this.allChartDropdownVlaues.map(x => {
  //     if (x.chartName == chartName) {
  //       res = x['chartKeyStringsList'][0]['filterString'];
  //     }
  //   })
