...
(function ($) {
			...
			if (!$row.hasClass('header')) {
				...
				if (currentLevel != prevLevel) {
					...
					if (currentLevel == self.maxLevel) {
						$row.children('.expander').css({ display: "none" })
					}
				}
	...
	//this method toggles the display level on or off, including a sliding motion
	self.setDisplayLevel = function (displayLvl) {
		self.caller.find('tr').each(function () {
			$element = $(this);
			if (!$element.hasClass('header')) {
				var rowDisplayLevel = $element.data('level');
				if (rowDisplayLevel <= displayLvl) {
					if (rowDisplayLevel < displayLvl) {
						$element.removeClass('contracted').addClass('expanded');
						$element.removeClass('hidden');
						$element.find('td').wrapInner('<div style="display: none;" />').parent().find('td > div').slideDown(100, function () {
							var $set = $(this);
							$set.replaceWith($set.contents());
						});
					} else {
					...
					$element.removeClass('expanded').addClass('contracted').addClass('hidden');
					// $element.find('td').wrapInner('<div style="" />').parent().find('td > div').slideUp(100, function () {
					// 	var $set = $(this);
					// 	$element.replaceWith($set.contents());
	...
	self.getPublicObj = function () {
		return {
			...
			setDisplayLevel: self.setDisplayLevel,
